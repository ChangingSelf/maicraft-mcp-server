...
- 配置github action，在合并到main分支时发布到npm

## [1.10.5] - 2025-10-09

- 修复npm包中`minecrafthawkeye`插件动态导入失败导致事件系统无法正常工作的问题
- 使用`createRequire`替代动态导入，并添加动态导入作为fallback
- 简化hawkEye插件加载逻辑，直接使用`hawkEyeModule.default`
- 提前初始化事件管理器和事件监听器（在spawn事件之前）
- 优化日志输出，减少冗余信息，保留关键状态信息
- 改进hawkEye插件加载的错误处理，确保插件加载失败不会影响其他功能

## [1.10.2] - 2025-10-06

- 修复`minecrafthawkeye`插件的导入错误

## [1.10.1] - 2025-09-26

- 优化 `killMobAction` ，允许远程攻击，并且攻击时会自动使用合适的武器
- 添加自动穿戴护甲的机制

## [1.10.0] - 2025-09-25

- 移除已不再使用的截图动作和ViewManager及其相关依赖
