...
import type { MaicraftPayload } from "./PayloadTypes.js";
...
export interface WebSocketClientEvents {
  ...
  message: (message: MaicraftPayload) => void;
...
export class WebSocketClient extends EventEmitter {
      ...
      try {
        ...
        const timeout = setTimeout(() => {
          if (this.ws?.readyState === WebSocket.CONNECTING) {
            ...
            this.logger.error("WebSocket 连接超时");
        ...
        this.ws.onerror = (error: WebSocket.ErrorEvent) => {
          ...
          const connectionError = new Error(`WebSocket 连接错误: ${errorMessage}`);
          
          this.logger.error("WebSocket 连接错误:", errorMessage);
   ...
   * 发送消息
   */
  async sendMessage(payload: MaicraftPayload): Promise<void> {
    ...
    try {
      ...
      this.logger.debug("消息已发送:", payload.message_id || "no-id");
    } catch (error) {
  ...
  private handleMessage(data: string): void {
    try {
      ...
      // 验证并处理 MaicraftPayload
      try {
        const payload = PayloadValidator.validateIncomingPayload(messageData);
        this.logger.info("收到载荷消息:", payload.message_id || "no-id");
        this.emit("message", payload);
        return;
      } catch (validationError) {
        this.logger.warn("消息验证失败:", validationError);
        return;
      }
